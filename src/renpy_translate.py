...
from html_util import (
    plain_text_to_html_from_list,
    read_strings_from_html,
    read_strings_from_translated,
...
from google_page_translate import GooglePageTranslate
...
engineList = [
    ...
    "Google(New API)",
    "YouDao(Token Required)",
    "DeepL(Token Required)",
    "OpenAI(Token Required)",
    "Alibaba(Free)",
    "ModernMt(Free)",
    "Bing(Free)",
    "Lingvanex(Free)",
    "CloudTranslation(Free)",
    "YouDao(Free)",
    "Caiyun(Free)",
    "Webbrower(Custom)",
]
...
engineDic = {
    ...
    # engineList[5]: {'url': 'https://platform.openai.com/api-keys', 'key_edit': True,
    ...
    # engineList[9]: {'url': 'https://lingvanex.com/demo', 'key_edit': False,
    ...
    # engineList[12]: {'url': 'https://fanyi.caiyunapp.com/', 'key_edit': False,
...
class translateThread(threading.Thread):
    def __init__(
        ...
        lang_target,
        lang_source,
        is_open_multi_thread,
        is_gen_bak,
        local_glossary,
        is_translate_current,
        is_skip_translated,
        is_open_filter,
        filter_length,
        is_replace_special_symbols,
    ):
    ...
    def run(self):
        ...
        try:
            ...
            self.TranslateFile(
                ...
                self.is_gen_bak,
                self.local_glossary,
                self.is_translate_current,
                self.is_skip_translated,
                self.is_open_filter,
                self.filter_length,
                self.is_replace_special_symbols,
            )
    ...
    def TranslateFile(
        ...
        if (
            client.__class__.__name__ == "Translate"
            and local_glossary is not None
            and len(local_glossary) > 0
        ):
            fmt = "html"
        else:
            fmt = "text"
        ...
        if isinstance(client, str) and client == "web_brower":
            plain_text_to_html_from_list(
                transList, web_brower_export_name, is_replace_special_symbols
            )
            return
        trans_dic = TranslateToList(
            client, transList, lang_target, lang_source, fmt=fmt
        )
            ...
            if translated is None:
                ...
                if target == current:
                    if _read_lines[line].startswith("    new "):
                        ...
                        _read_lines[line] = header + content.replace(
                            target, translated, 1
                        )
                    else:
                        _read_lines[line] = _read_lines[line].replace(
                            target, translated, 1
                        )
                else:
                    _read_lines[line] = (
                        "    "
                        + _read_lines[ori_line]
                        .replace(target, translated)
                        .lstrip()
                        .lstrip("#")
                        .lstrip()
                    )
        ...
        with open("engine.txt", "r", encoding="utf-8") as json_file:
            ...
            if loaded_data["engine"] == engineList[0]:
                ...
                client = ApiKeyTranslate(
                    fmt="text", proxies=proxies, api_key=loaded_data["key"]
                )
            elif loaded_data["engine"] == engineList[2]:
                client = GooglePageTranslate(proxies=proxies)
            elif loaded_data["engine"] == engineList[3]:
                client = YoudaoTranslate(
                    app_key=loaded_data["key"],
                    app_secret=loaded_data["secret"],
                    proxies=proxies,
                )
                ...
                if proxies is None:
                    ...
                    proxies["https"] = None
                if (
                    "openai_base_url" in loaded_data
                    and len(loaded_data["openai_base_url"]) > 0
                ):
                    base_url = loaded_data["openai_base_url"]
                global client_openai
                if (
                    "time_out" not in loaded_data
                    or len(loaded_data["openai_model"]) == 0
                ):
                ...
                if (
                    "max_length" not in loaded_data
                    or len(loaded_data["max_length"]) == 0
                ):
                    loaded_data["max_length"] = 5000

                if client_openai is None:
                    client_openai = OpenAITranslate(
                        app_key=loaded_data["key"],
                        rpm=loaded_data["rpm"],
                        rps=loaded_data["rps"],
                        tpm=loaded_data["tpm"],
                ...
                client = CustomTranslate(
                    custom_header + config["file_name"],
                    loaded_data["key"],
                    loaded_data["secret"],
                    proxies,
                    config["is_queue"],
                )
            elif loaded_data["engine"] == engineList[13]:
                client = "web_brower"
            else:
...
def get_translated(trans_dic, d):
    try:
        if not isAllPunctuations(d["encoded"].strip('"')):
            ...
            dd = DecodeBrackets(translated, d["en_1"], d["en_2"], d["en_3"])
            if (
                d["en_1_cnt"] != dd["de_6_cnt"]
                or d["en_2_cnt"] != dd["de_5_cnt"]
                or d["en_3_cnt"] != dd["de_4_cnt"]
            ):
    ...
    try:
        ...
        for line_index, line_content in enumerate(_read_line):
            ...
            if line_content.strip().startswith(
                'old _p("""'
            ) or line_content.strip().startswith('new _p("""'):
                is_p = True
            if is_p:
            ...
            if not isVoice:
                if (
                    line_index > 0
                    and not _read_line[line_index - 1].strip().startswith("#")
                    and not _read_line[line_index - 1].strip().startswith("old ")
                ):
                    continue
                ...
                for i, e in enumerate(d["oriList"]):
                    if isAllPunctuations(d["encoded"].strip('"')) == False:
                        ...
                        if line_content.strip().startswith("new "):
                            d_o = EncodeBracketContent(
                                _read_line[target_index].strip()[4:], '"', '"'
                            )
                        else:
                            d_o = EncodeBracketContent(
                                _read_line[target_index].strip(), '"', '"'
                            )
                        original = ""
